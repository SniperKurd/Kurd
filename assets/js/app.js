(async function () {
  ...
  try {
    ...
    const price = reserve1 && reserve0 ? (reserve1 / reserve0).toFixed(6) : 'N/A';
    document.getElementById('pairInfo').innerHTML = `<strong>${name0} (${sym0}) / ${name1} (${sym1})</strong><br>Price: 1 ${sym0} = ${price} ${sym1}`;
  } catch (err) {
  ...
  document.getElementById('swapBtn').addEventListener('click', async function () {
    ...
    const forceSwap = document.getElementById('forceSwap').checked;
    if (!amountStr) {
    ...
    try {
      ...
      const minOut = forceSwap ? 0n : amountOut - (amountOut * BigInt(Math.floor(slip * 100)) / 10000n);
